...
const hash = await sourceHash(), strategies = Object.keys(SENSOR_ARMS) as SensorArm[], directory = resolve(args.get('output') ?? `.runtime/experiments/jev-sensors-${Date.now()}`);
if (phase === 'held-out') {
...
const manifest = { phase, recordedAt: new Date().toISOString(), model: MODEL, sourceHash: hash, strategies, definitions: SENSOR_ARMS, seeds, seconds,
  configs: Object.fromEntries(strategies.map(s => [s, config])), marker: MARKER, scoring: config.scoring,
  design: 'Matched sensor ablation: rectified camera pixels processed with js-aruco2 detection and POSIT, versus that same camera plus one TF-Luna. Same six fixed control menus and English goals. No target broadcast, global odometry, point cloud, simulated camera depth, contact oracle, future route or evaluator feedback. No scene-based command guard. World continues at 50 Hz; camera acquires at 5 Hz, TF-Luna at 50 Hz.',
  order: 'Rotate arm order by seed. Keep every outcome. Development qualifies interfaces, not selection of a winning policy.',
  limitations: ['The target carries a known printed 0.4 m ARUCO_MIP_36h12 marker on its roof. This is an explicit instrumented-target experiment, not general object recognition. The pattern and mounting knowledge would also be required on hardware.',
    ...
    'TF-Luna uses nine cone rays, reflectivity-limited signal, centimetre quantization and measurement error. Returns blend by estimated amplitude; partial and mixed surfaces can give misleading VALID distances. There is no perfect mixed-return rejection. This approximate optical response and absent sunlight/weather model are not hardware calibration.',
    'One fixed body-mounted TF-Luna pitches 30 degrees down. It does not track camera pitch and never supplies object identity. Its beam must not be confused with camera target range.',
    ...
    'Legacy trace field source.odometryMs means camera acquisition time in these runs. Receipt and application clocks remain separate. Default real TF-Luna UART has no device timestamp; real adapters must report host receipt and estimated acquisition uncertainty honestly.',
    'Marker detection and physical packet decoding are reusable sensor processing, not a mission policy. Jev chooses all six controls. No automatic follow, aiming, obstacle avoidance, candidate ranking or output substitution.',
  ...
  sources: ['https://www.dfrobot.com/product-1995.html', 'https://en.benewake.com/uploadfiles/2025/04/20250430174515390.pdf', 'https://github.com/damianofalcioni/js-aruco2', 'https://mavlink.io/en/messages/common.html#DISTANCE_SENSOR'] };
