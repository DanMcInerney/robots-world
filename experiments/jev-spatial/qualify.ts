...
import {CASES,layout,scoutEnvironment,insideObstacle} from '../jev-scout/scenario.ts';

const root=resolve(process.argv[2]??'.runtime/experiments/jev-spatial-five-rounds-v1/qualification-base-v1');
await mkdir(root); // Never overwrite evidence.
const rows:any[]=[];
for(const seed of [3101,3102])for(const kind of CASES){
