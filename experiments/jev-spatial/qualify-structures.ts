...
import {spatialProfile as recordedRgbProfile} from './profile.ts';
...
import {insideObstacle} from '../jev-scout/scenario.ts';
import {spatialLayout,spatialEnvironment} from './fixtures.ts';

const root=resolve(process.argv[2]??'.runtime/experiments/jev-spatial-five-rounds-v1/qualification-structures-v1');
await mkdir(root); // Never overwrite evidence.
const rows:any[]=[];
for(const seed of [3701,3702])for(const [fixture,kind] of [['screen-return-v1','behind-wall'],['offset-piers-v1','occlusion-course']] as const){
  const id=`${fixture}-${seed}`,spec=spatialLayout(seed,kind,fixture),dir=resolve(root,'frames',id),frames:any[]=[];
  ...
  const world=await ReactiveWorld.create(seed,()=>{},1e12,config,profile,task,spatialEnvironment(seed,kind,fixture));
  try{
    ...
    assert(!scan.regions.some(o=>o.color==='blue'),'Wall fixture solvable by rotation at spawn');
    ...
    const row={id,kind,fixture,seed,pathClear,seconds:120,firstDeliveredMs:initial.sensors.camera.acquiredSimMs,initialTargetVisible:false,holdVisibleFrames:visible.length,acquiredFrames:frames.length,
...
await copyFile('experiments/jev-spatial/qualification-preview.html',resolve(root,'index.html'));
