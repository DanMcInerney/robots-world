...
**2026-09-21 forward pointer:** the servo-computed camera aiming this cohort's 6/8 passes depended on is superseded by F57–F61's code-computed per-option consequences, which let Jev itself choose the action instead. F75 later found the same consequence approach also gives the only useful search-menu competence (`sector-consequences`). See the [find-and-follow ladder](jev-find-follow-ladder.md) L1–L2, which reuses this cohort's staged structure with the consequence encoding in place of the servo.
