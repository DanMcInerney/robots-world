...
**Narrowed supersession note, 2026-09-21:** only the **servo** (declared camera-bearing servo performing Jev's aiming arithmetic, § "One baseline and a staged progression" below) is superseded — by F57–F61's code-computed per-option consequence encoding, which lets Jev itself choose the yaw action instead of a servo computing it. The **staging** this document proposes (centre a visible target → add one pursuit axis → add temporary loss → add obstacles) is reinstated: it is exactly the structure the [find-and-follow ladder](jev-find-follow-ladder.md) (F78) follows (L1 hold-visible → L2/L3/L4 add motion/range/coupling → L6/L7 add loss/identity → L9 adds obstacles and initial invisibility), with the consequence encoding standing in for the servo at every rung. The gates this document proposes (lock within 5 s; ≥95% centred seconds 5–60; no loss >1 s) are not reused verbatim — both use a 60 s-scale window, but the ladder derives its central-band tolerance from a measured reference-controller floor rather than asserting one, and L1 itself runs many shorter (20 s) episodes across an offset×range sweep rather than one long 60 s hold, per a second review's finding that a single static episode is degenerate — but the staging order is.
