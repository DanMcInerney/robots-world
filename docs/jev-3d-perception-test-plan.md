...
**2026-09-21 note on test 2 (distance wording):** superseded — F75 ran this comparison (`signed-error` vs. a per-option `after-range` alternative not anticipated here) and found `after-range` robust (116/116 across both splits) where `signed-error` failed confirmation (4 harmful choices) despite an identical development score. Also, the 4–18 m target-range setting proposed just below (§ "Small experiments") was never reached: the range actually rendered across all 1,200 Round 3 frames was verified as 5.15–14.998 m (`.runtime/experiments/jev-round3-v1/diagnostics.json`), so distances beyond ~15 m remain untested, not merely out of the proposed range. See the [find-and-follow ladder](jev-find-follow-ladder.md) L3a/L3b/L4/L8.
